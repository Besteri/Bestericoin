...
const string strMessageMagic = "Bestericoin Signed Message:\n";
...
{
    // Bestericoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // Bestericoin: 1 day
static const int64 nTargetSpacing = 2.5 * 60; // Bestericoin: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bestericoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Bestericoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x89982e2c96b72ebd943a5eef78e205254174f4fd0e366ebafa6db2b71d731cd7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "Bestericoin for You";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xee, 0xb3, 0xa9, 0xce }; // Bestericoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BestericoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BestericoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BestericoinMiner(CWallet *pwallet)
{
    printf("BestericoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bestericoin-miner");
    ...
    try { loop {
        ...
        printf("Running BestericoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BestericoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BestericoinMiner, pwallet));
}
